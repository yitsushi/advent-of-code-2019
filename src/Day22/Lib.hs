...
parseLine line | take 3 line == "cut"       = Cut (read $ drop 4 line)
...
shuffle deck (Cut n) | n > 0     = drop n deck ++ take n deck
                     | otherwise = drop (l + n) deck ++ take (l + n) deck
  where l = length deck
shuffle deck (Increment n) = map snd $ L.sortBy order $ zipWith
  (curry placement)
  [0 ..]
  deck
 where

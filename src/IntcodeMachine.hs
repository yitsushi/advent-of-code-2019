...
comparison c (p1, p2, p3) = do
  v1 <- resolveParam p1
  v2 <- resolveParam p2
  write p3 $ if c v1 v2 then 1 else 0
...
newComputer tape inputs = Computer { memory = HM.fromList $ zip [0 ..] tape
                                   , ip           = 0
                                   , input        = inputs
                                   , output       = []
                                   , relativeBase = 0
...
cleanComputerWithOutput out = Computer { memory       = HM.fromList []
                                       , ip           = 0
                                       , input        = []
                                       , output       = out
                                       , relativeBase = 0
...
writeMemory location value computer = computer { memory = newMemory }
  where newMemory = HM.insert location value (memory computer)

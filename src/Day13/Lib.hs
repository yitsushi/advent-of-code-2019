...
parseGameScreen game@Game { gameProgram = gp, gameScreen = gs } = game'
 where
  ...
  game'      = game
    ...
    , gameScore = (score . map snd . Map.toList . Map.filter scoreFilter) screen
    }
   where
...
displayGame game = (:) scoreLine $ map (map tileChar) $ partition
  width
  [ tileAt game (x, y)
  | y <- reverse [miny .. maxy]
  , x <- reverse [minx .. maxx]
...
newGame p c = Game { gameProgram    = p
                   , gameController = c

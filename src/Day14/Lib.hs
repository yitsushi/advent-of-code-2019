...
requiredOreFor db pocket name qty = pocket'
  where
    item =
      fromMaybe (error ("I dont't know this iteam: " ++ name)) $
      Map.lookup name db
    ...
    pocket' =
      foldl
        ...
        (rIngredients item)
    walk p ing
      | riName ing == "ORE" = addItem (riName ing) required p
...
parseRecipe line = resourceFromTuple result ingredients
  where
    ...
    separate l = (read qty :: Int, name)
      where
        [qty, name] = splitOn ' ' $ dropWhile (== ' ') l
    ingredients =
      (map (ingredientFromTuple . separate) .
       splitOn ',' . init . takeWhile (/= '='))
        line

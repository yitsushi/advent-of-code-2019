...
executeBeamDetector detector@BeamDetector { bdProgram = computer
                                          , bdSpace = space
                                          , bdController = controller
                                          }
  | isTerminated computer = detector
  | isNothing input = detector
  | otherwise = executeBeamDetector detector' {bdProgram = computer}
  where
...
parseResponse detector@BeamDetector { bdProgram = computer
                                    , bdSpace = space
                                    , bdLastInput = input
                                    , beamStartAt = beamStartAt
  ...
  where
    ...
    space' =
      case input of
        ...
        Nothing -> space
    oPoint =
      if isNothing originPoint && value == Beam
        then input
        else originPoint
    bPoint =
      if isJust originPoint && isNothing beamStartAt && value == Beam
        then input
        else beamStartAt
